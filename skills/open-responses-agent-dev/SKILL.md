...
| **Rust** | High-performance, systems, embedded | `reqwest` (raw HTTP) |
...
For Rust, use raw HTTP with `reqwest` since there's no official Rust SDK.

### Setup (Cargo.toml)

```toml
[package]
name = "open-responses-agent"
version = "0.1.0"
edition = "2021"
...
async fn create_agent(
    ...
    input: &str,
    instructions: &str,
) -> Result<OpenResponsesResponse, reqwest::Error> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Or process all output items
    for item in &result.output {
        match item.item_type.as_str() {
            "reasoning" => {
                println!("Thinking: {}", item.content.as_deref().unwrap_or(""));
            }
            "message" => {
                println!("Response: {}", item.content.as_deref().unwrap_or(""));
            }
            _ => {}
        }
    }
...
### Sub-Agent Loop with Tools (Rust)

```rust
async fn run_agent_with_tools(api_key: &str) -> Result<OpenResponsesResponse, reqwest::Error> {
    let client = Client::new();

    // Tools defined at top level
    let tools = json!([
        {
            "type": "function",
            "name": "get_current_weather",
    ...
    for item in &response.output {
        match item.item_type.as_str() {
            "reasoning" => println!("[REASONING] {}", item.content.as_deref().unwrap_or("")),
            "function_call" => println!("[TOOL CALL] {} - {:?}", item.name.as_deref().unwrap_or(""), item.arguments),
            "function_call_output" => println!("[TOOL RESULT] {}", item.output.as_deref().unwrap_or("")),
            "message" => println!("[RESPONSE] {}", item.content.as_deref().unwrap_or("")),
            _ => {}
        }
    }
